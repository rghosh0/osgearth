...
PagedLODWithNodeOperations::PagedLODWithNodeOperations(
    RefNodeOperationVector *postMergeOps)
    : _postMergeOps(postMergeOps), _visibilityMaxAltitude(FLT_MAX),
      _currentAltitude(FLT_MAX), _isVisible(true) {
  // nop
}
...
void PagedLODWithNodeOperations::runPostMerge(osg::Node *node) {
  if (_postMergeOps.valid()) {
    ...
    for (NodeOperationVector::iterator i = _postMergeOps->begin();
         i != _postMergeOps->end(); ++i) {
      i->get()->operator()(node);
    }
...
void PagedLODWithNodeOperations::setVisibilityMaxAltitude(
...
void PagedLODWithNodeOperations::traverse(osg::NodeVisitor &nv) {
    ...
    for (unsigned i = 0; i < group.getNumChildren(); ++i) {
      ...
      if (child) {
        if (std::string(child->className()).compare("Group") == 0 &&
            child->getStateSet() == 0L && child->getCullCallback() == 0L &&
            child->getUpdateCallback() == 0L && child->getUserData() == 0L &&
            child->getName().empty() && child->getDescriptions().size() == 0) {
...
PrimitiveSetTypeCounter::PrimitiveSetTypeCounter()
    : osg::NodeVisitor(osg::NodeVisitor::TRAVERSE_ALL_CHILDREN), _point(0),
      _line(0), _polygon(0) {
  // nop
}

void PrimitiveSetTypeCounter::apply(osg::Drawable &drawable) {
  osg::Geometry *g = drawable.asGeometry();
  if (g) {
    const osg::Geometry::PrimitiveSetList &primSets = g->getPrimitiveSetList();
    for (osg::Geometry::PrimitiveSetList::const_iterator j = primSets.begin();
         j != primSets.end(); ++j) {
...
struct TriangleIntersection {
  TriangleIntersection(unsigned int index, const osg::Vec3d &normal, double r1,
                       const osg::Vec3 *v1, double r2, const osg::Vec3 *v2,
                       double r3, const osg::Vec3 *v3)
      : _index(index), _normal(normal), _r1(r1), _v1(v1), _r2(r2), _v2(v2),
        _r3(r3), _v3(v3) {}
...
struct DoublePrecisionTriangleIntersector {
  ...
  inline void operator()(const osg::Vec3 &v1, const osg::Vec3 &v2,
    ...
    double r3;
    if (osg::equivalent(ds12, 0.0))
      r3 = 0.0;
    else if (!osg::equivalent(d312, 0.0))
      r3 = ds12 / d312;
    else
      return; // the triangle and the line must be parallel intersection.

    double r1;
    if (osg::equivalent(ds23, 0.0))
      r1 = 0.0;
    else if (!osg::equivalent(d123, 0.0))
      r1 = ds23 / d123;
    else
      return; // the triangle and the line must be parallel intersection.

    double r2;
    if (osg::equivalent(ds31, 0.0))
      r2 = 0.0;
    else if (!osg::equivalent(d231, 0.0))
      r2 = ds31 / d231;
    else
      return; // the triangle and the line must be parallel intersection.
    ...
    if (treatVertexDataAsTemporary) {
      _intersections.insert(std::pair<const double, TriangleIntersection>(
          r, TriangleIntersection(_index - 1, normal, r1, 0, r2, 0, r3, 0)));
    } else {
      _intersections.insert(std::pair<const double, TriangleIntersection>(
          r,
          TriangleIntersection(_index - 1, normal, r1, &v1, r2, &v2, r3, &v3)));
    }
    _hit = true;
  }
};
...
ObserverGroup::ObserverGroup() { ADJUST_EVENT_TRAV_COUNT(this, 1); }

void ObserverGroup::traverse(osg::NodeVisitor &nv) {
  if (nv.getVisitorType() == nv.EVENT_VISITOR) {
    ...
    // delete orphans:
    for (std::set<osg::Node *>::iterator i = _orphans.begin();
         i != _orphans.end(); ++i) {
      this->removeChild(*i);
      ADJUST_UPDATE_TRAV_COUNT(this, -1);
    }

    OE_DEBUG << LC << _orphans.size() << " orphaned children removed."
             << std::endl;
    _orphans.clear();
  }
