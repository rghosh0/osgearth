...
FeatureLevel::FeatureLevel(const Config &conf)
    : _minRange(0.0f), _maxRange(FLT_MAX), _maxVisibilityAltitude(FLT_MAX) {
  fromConfig(conf);
}
...
void FeatureLevel::fromConfig(const Config &conf) {
...
FeatureDisplayLayout::FeatureDisplayLayout(const Config &conf)
    : _tileSizeFactor(15.0f), _minRange(0.0f), _maxRange(0.0f),
      _cropFeatures(false), _priorityOffset(0.0f), _priorityScale(1.0f),
      _minExpiryTime(0.0f), _paged(true) {
  fromConfig(conf);
}

void FeatureDisplayLayout::fromConfig(const Config &conf) {
...
unsigned FeatureDisplayLayout::getNumLevels() const { return _levels.size(); }

const FeatureLevel *FeatureDisplayLayout::getLevel(unsigned n) const {
  unsigned i = 0;
