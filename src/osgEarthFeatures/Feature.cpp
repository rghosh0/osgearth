...
FeatureProfile::FeatureProfile(const GeoExtent &extent)
    : _extent(extent), _firstLevel(0), _maxLevel(-1), _tiled(false) {
  // nop
}

bool FeatureProfile::getTiled() const { return _tiled; }

void FeatureProfile::setTiled(bool tiled) { _tiled = true; }

int FeatureProfile::getFirstLevel() const { return _firstLevel; }

void FeatureProfile::setFirstLevel(int firstLevel) { _firstLevel = firstLevel; }

int FeatureProfile::getMaxLevel() const { return _maxLevel; }

void FeatureProfile::setMaxLevel(int maxLevel) { _maxLevel = maxLevel; }
...
std::string AttributeValue::getString() const {
  ...
  case ATTRTYPE_DOUBLE:
    return osgEarth::toString(second.doubleValue);
  case ATTRTYPE_INT:
    return osgEarth::toString(second.intValue);
  case ATTRTYPE_BOOL:
    return osgEarth::toString(second.boolValue);
  case ATTRTYPE_DOUBLEARRAY:
    return osgEarth::toString(second.doubleArrayValue);
  case ATTRTYPE_UNSPECIFIED:
    break;
  }
  return EMPTY_STRING;
}
...
double AttributeValue::getDouble(double defaultValue) const {
  ...
  switch (first) {
  case ATTRTYPE_STRING:
    return osgEarth::as<double>(second.stringValue, defaultValue);
  case ATTRTYPE_DOUBLE:
    return second.doubleValue;
  case ATTRTYPE_INT:
    return (double)second.intValue;
  case ATTRTYPE_BOOL:
    return second.boolValue ? 1.0 : 0.0;
  case ATTRTYPE_DOUBLEARRAY:
    break;
  case ATTRTYPE_UNSPECIFIED:
    break;
...
int AttributeValue::getInt(int defaultValue) const {
  ...
  switch (first) {
  case ATTRTYPE_STRING:
    return osgEarth::as<int>(second.stringValue, defaultValue);
  case ATTRTYPE_DOUBLE:
    return (int)second.doubleValue;
  case ATTRTYPE_INT:
    return second.intValue;
  case ATTRTYPE_BOOL:
    return second.boolValue ? 1 : 0;
  case ATTRTYPE_DOUBLEARRAY:
    break;
  case ATTRTYPE_UNSPECIFIED:
    break;
...
bool AttributeValue::getBool(bool defaultValue) const {
  ...
  switch (first) {
  case ATTRTYPE_STRING:
    return osgEarth::as<bool>(second.stringValue, defaultValue);
  case ATTRTYPE_DOUBLE:
    return second.doubleValue != 0.0;
  case ATTRTYPE_INT:
    return second.intValue != 0;
  case ATTRTYPE_BOOL:
    return second.boolValue;
  case ATTRTYPE_DOUBLEARRAY:
    break;
  case ATTRTYPE_UNSPECIFIED:
    break;
...
osg::ref_ptr<osg::DoubleArray> AttributeValue::getDoubleArray() const {
  switch (first) {
  case ATTRTYPE_STRING:
    break;
  case ATTRTYPE_DOUBLE:
...
Feature::Feature(Geometry *geom, const SpatialReference *srs,
                 const Style &style, FeatureID fid)
    : _geom(geom), _srs(srs), _fid(fid) {
  if (!style.empty())
    _style = style;
...
Feature::Feature(const Feature &rhs, const osg::CopyOp &copyOp)
    : _fid(rhs._fid), _attrs(rhs._attrs), _style(rhs._style),
      _geoInterp(rhs._geoInterp), _srs(rhs._srs.get()) {
  if (rhs._geom.valid())
...
FeatureID Feature::getFID() const { return _fid; }

void Feature::setFID(FeatureID fid) { _fid = fid; }

GeoExtent Feature::getExtent() const {
...
void Feature::setSRS(const SpatialReference *srs) {
  _srs = srs;
  dirty();
}

void Feature::setGeometry(Geometry *geom) {
  _geom = geom;
  dirty();
}
...
void Feature::set(const std::string &name, const std::string &value) {
...
void Feature::set(const std::string &name, double value) {
  AttributeValue &a = _attrs[name];
...
void Feature::set(const std::string &name, int value) {
  AttributeValue &a = _attrs[name];
...
void Feature::set(const std::string &name, const AttributeValue &value) {
  _attrs[name] = value;
}

void Feature::set(const std::string &name, bool value) {
  AttributeValue &a = _attrs[name];
...
void Feature::set(const std::string &name,
                  osg::ref_ptr<osg::DoubleArray> value) {
  AttributeValue &a = _attrs[name];
...
void Feature::setNull(const std::string &name) {
  AttributeValue &a = _attrs[name];
  a.second.set = false;
}

void Feature::setNull(const std::string &name, AttributeType type) {
  AttributeValue &a = _attrs[name];
  a.first = type;
  a.second.set = false;
}
...
double Feature::getDouble(const std::string &name, double defaultValue) const {
  AttributeTable::const_iterator i = _attrs.find(toLower(name));
  return i != _attrs.end() ? i->second.getDouble(defaultValue) : defaultValue;
}

int Feature::getInt(const std::string &name, int defaultValue) const {
  AttributeTable::const_iterator i = _attrs.find(toLower(name));
  return i != _attrs.end() ? i->second.getInt(defaultValue) : defaultValue;
}

bool Feature::getBool(const std::string &name, bool defaultValue) const {
  AttributeTable::const_iterator i = _attrs.find(toLower(name));
...
Feature::getDoubleArray(const std::string &name) const {
  AttributeTable::const_iterator i = _attrs.find(toLower(name));
  return i != _attrs.end() ? i->second.getDoubleArray() : nullptr;
}

bool Feature::isSet(const std::string &name) const {
  AttributeTable::const_iterator i = _attrs.find(toLower(name));
  return i != _attrs.end() ? i->second.second.set : false;
}

double Feature::eval(NumericExpression &expr,
                     FilterContext const *context) const {
  const NumericExpression::Variables &vars = expr.variables();
  for (NumericExpression::Variables::const_iterator i = vars.begin();
       i != vars.end(); ++i) {
    double val = 0.0;
    ...
    if (ai != _attrs.end()) {
      ...
      if (engine) {
        ...
        if (result.success())
          ...
          OE_WARN << LC << "Feature Script error on '" << expr.expr()
...
double Feature::eval(NumericExpression &expr, Session *session) const {
  const NumericExpression::Variables &vars = expr.variables();
  for (NumericExpression::Variables::const_iterator i = vars.begin();
       i != vars.end(); ++i) {
    double val = 0.0;
...
const std::string &Feature::eval(StringExpression &expr,
                                 FilterContext const *context) const {
  const StringExpression::Variables &vars = expr.variables();
  for (StringExpression::Variables::const_iterator i = vars.begin();
       i != vars.end(); ++i) {
    std::string val = "";
    AttributeTable::const_iterator ai = _attrs.find(toLower(i->first));
    if (ai != _attrs.end()) {
...
bool Feature::getWorldBound(const SpatialReference *srs,
...
bool Feature::getWorldBoundingPolytope(const SpatialReference *srs,
...
bool Feature::getWorldBoundingPolytope(const osg::BoundingSphered &bs,
  ...
  if (bs.valid()) {
    ...
    out_polytope.add(
        osg::Plane(osg::Vec3d(1, 0, 0), osg::Vec3d(-bs.radius(), 0, 0)));
    out_polytope.add(
        osg::Plane(osg::Vec3d(-1, 0, 0), osg::Vec3d(bs.radius(), 0, 0)));
    out_polytope.add(
        osg::Plane(osg::Vec3d(0, 1, 0), osg::Vec3d(0, -bs.radius(), 0)));
    out_polytope.add(
        osg::Plane(osg::Vec3d(0, -1, 0), osg::Vec3d(0, bs.radius(), 0)));

    // for a projected feature, we're done. For a geocentric one, transform the
    // polytope into world (ECEF) space.
    if (srs->isGeographic()) {
      ...
      if (zeroOffset > minRad * 0.1) {
        out_polytope.add(osg::Plane(osg::Vec3d(0, 0, 1),
                                    osg::Vec3d(0, 0, -maxRad + zeroOffset)));
      }
    }
...
GeoExtent Feature::calculateExtent() const {
  GeoExtent e(getSRS());
...
std::string Feature::getGeoJSON() const {
  std::string geometry = GeometryUtils::geometryToGeoJSON(getGeometry());

  Json::Value root(Json::objectValue);
  root["type"] = "Feature";
  root["id"] =
      (unsigned int)getFID(); // TODO:  Update JSON to use unsigned longs
  ...
  if (getAttrs().size() > 0) {
    ...
    for (AttributeTable::const_iterator itr = getAttrs().begin();
         itr != getAttrs().end(); ++itr) {
...
std::string Feature::featuresToGeoJSON(const FeatureList &features) {
  ...
  for (FeatureList::const_iterator i = features.begin(); i != features.end();
       i++) {
...
void Feature::splitAcrossDateLine(FeatureList &splitFeatures) {
  ...
  if (getSRS() && getSRS()->isGeodetic()) {
    ...
    if (extent.crossesAntimeridian()) {
      // This tries to split features across the dateline in three different
      // zones.  -540 to -180, -180 to 180, and 180 to 540.
      double minLon = -540;
      for (int i = 0; i < 3; i++) {
        ...
        if (getGeometry()->crop(bounds, croppedGeometry)) {
          // If the geometry was cropped, offset the x coordinate so it's within
          // normal longitude ranges.
          ...
          croppedGeometry->rewind(
              osgEarth::Symbology::Geometry::ORIENTATION_CCW);
