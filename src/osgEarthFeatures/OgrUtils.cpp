...
#define GDAL_VERSION_AT_LEAST(MAJOR, MINOR, REV)                               \
  ((GDAL_VERSION_MAJOR > MAJOR) ||                                             \
   (GDAL_VERSION_MAJOR == MAJOR &&                                             \
    (GDAL_VERSION_MINOR > MINOR ||                                             \
     (GDAL_VERSION_MINOR == MINOR && GDAL_VERSION_REV >= REV))))
#endif
...
int IsFieldSet(OGRFeatureH handle, int i) {
  ...
  // GDAL 2.2 changed the behavior of OGR_F_IsFieldSet so that null fields will
  // still be considered set. We consider unset or null fields to be the same,
  // so we use OGR_F_IsFieldSetAndNotNull
...
void OgrUtils::populate(OGRGeometryH geomHandle, Symbology::Geometry *target,
                        int numPoints) {
...
Symbology::Polygon *OgrUtils::createPolygon(OGRGeometryH geomHandle) {
...
Symbology::Geometry *OgrUtils::createGeometry(OGRGeometryH geomHandle) {
  ...
  case wkbMultiPolygonZM:
    ...
    for (int n = 0; n < numGeoms; n++) {
      ...
      if (subGeomRef) {
        ...
        if (geom)
          multi->getComponents().push_back(geom);
      }
    }
    output = multi;
    break;
...
OGRGeometryH OgrUtils::encodePart(const Geometry *geometry,
...
OGRGeometryH OgrUtils::encodeShape(const Geometry *geometry,
...
OGRwkbGeometryType OgrUtils::getOGRGeometryType(
    const osgEarth::Symbology::Geometry::Type &geomType) {
...
OgrUtils::getOGRGeometryType(const osgEarth::Symbology::Geometry *geometry) {
  ...
  case Geometry::TYPE_MULTI: {
    const osgEarth::Symbology::MultiGeometry *multi =
        dynamic_cast<const MultiGeometry *>(geometry);
    if (multi) {
      osgEarth::Symbology::Geometry::Type componentType =
          multi->getComponentType();
      requestedType = componentType == Geometry::TYPE_POLYGON
                          ? wkbMultiPolygon
                          : componentType == Geometry::TYPE_POINTSET
                                ? wkbMultiPoint
                                : componentType == Geometry::TYPE_LINESTRING
                                      ? wkbMultiLineString
                                      : wkbNone;
    }
  } break;
...
OGRGeometryH
OgrUtils::createOgrGeometry(const osgEarth::Symbology::Geometry *geometry,
                            OGRwkbGeometryType requestedType) {
  if (!geometry)
    return NULL;
  ...
  OGRwkbGeometryType shape_type =
      requestedType == wkbPolygon || requestedType == wkbMultiPolygon
          ? wkbPolygon
          : requestedType == wkbPolygon25D ||
                    requestedType == wkbMultiPolygon25D
                ? wkbPolygon25D
                : requestedType == wkbLineString ||
                          requestedType == wkbMultiLineString
                      ? wkbMultiLineString
                      : requestedType == wkbLineString25D ||
                                requestedType == wkbMultiLineString25D
                            ? wkbMultiLineString25D
                            : requestedType == wkbPoint ||
                                      requestedType == wkbMultiPoint
                                  ? wkbMultiPoint
                                  : requestedType == wkbPoint25D ||
                                            requestedType == wkbMultiPoint25D
                                        ? wkbMultiPoint25D
                                        : wkbNone;

  OGRwkbGeometryType part_type =
      shape_type == wkbPolygon || shape_type == wkbPolygon25D
          ? wkbLinearRing
          : shape_type == wkbMultiLineString
                ? wkbLineString
                : shape_type == wkbMultiLineString25D
                      ? wkbLineString25D
                      : shape_type == wkbMultiPoint
                            ? wkbPoint
                            : shape_type == wkbMultiPoint25D ? wkbPoint25D
                                                             : wkbNone;

  // OE_NOTICE << "shape_type = " << shape_type << " part_type=" << part_type <<
  // std::endl;
  ...
  if (multi) {
    ...
    for (GeometryCollection::const_iterator itr =
             multi->getComponents().begin();
         itr != multi->getComponents().end(); ++itr) {
      OGRGeometryH shape_handle =
          encodeShape(itr->get(), shape_type, part_type);
      if (shape_handle) {
        ...
        if (error != OGRERR_NONE) {
          ...
          OE_WARN << "shape_type = " << shape_type << " part_type=" << part_type
                  << std::endl;
        }
      }
    }
...
Feature *OgrUtils::createFeature(OGRFeatureH handle,
  ...
  for (int i = 0; i < numAttrs; ++i) {
    ...
    case OFTRealList: {
      if (OGR_F_IsFieldSet(handle, i)) {
        int doubleNumber;
        const double *value =
            OGR_F_GetFieldAsDoubleList(handle, i, &doubleNumber);
        if ((doubleNumber) > 0) {
          osg::ref_ptr<osg::DoubleArray> valueArray = new osg::DoubleArray(
              static_cast<unsigned int>(doubleNumber), value);
